...
export default function Order() {
   ...
   const columns = [
      ...
      },
      {
         ...
         key: "action",
         render: (_, booking) => {
            // --- LOGIC KIỂM TRA QUÁ NGÀY CHO PENDING ---
            ...
            if (
               ...
               return (
                  <Button
                     ...
                     ghost
                     size="large"
                     onClick={() => handleShowPaidModal(booking)}
                  >
                     Chờ thanh toán
                  </Button>
               );
            ...
            return (
               <Tag icon={<QuestionOutlined />} color="warning">
                  Không có hành động
               </Tag>
            );
         },
   ...
   const fetchBookings = useCallback(async () => {
      ...
      if (!responseUser) {
         message.error(
            "Không lấy được không tin của bạn. Vui lòng thử lại sau!"
         );
         navigate("/user");
      }
      ...
      const checkUserId = responseUser.id;

      const response = await getAllBookingsByUserIdWithFilterPage(
         pageIndex,
         pageSize,
         statusParam,
         checkUserId
   ...
   const handleCancelBooking = async () => {
      ...
      try {
         const sendEmailResponse = await sendEmailPendingOrCancelOrPaidBooking(
            accountLogged.email,
            "CANCELED",
   ...
   const handleShowPaidModal = (booking) => {
   ...
   const handleClosePaidModal = () => {
      setIsShowPaidModal(false);
      setBaseId(null);
   };

   let currentPaidBooking = bookings?.find((booking) => booking.id === baseId);
   // Cập nhật lại currentPaidBooking khi thêm sửa xóa customer
   useEffect(() => {
      // Lấy booking hiện tại để thanh tonas
      currentPaidBooking = bookings?.find((booking) => booking.id === baseId);
   }, [baseId]);

   // Lấy tổng số khách hàng
   const paidCustomerCount = currentPaidBooking?.customers
      ? currentPaidBooking?.customers?.length
      : 0;

   // Giá tiền gốc
   const paidBasePrice = currentPaidBooking?.dayDetail?.price || 0;

   // Tổng tiền = khách hàng x tiền
   const padiTotalPrice = paidCustomerCount * paidBasePrice;

   // Hàm Thanh toán đơn đặt
   const handlePaidBooking = async () => {
      ...
      try {
         ...
         if (sendEmailResponse) {
            message.success(
               "Đã gửi Email thanh toán về địa chỉ email người đặt"
            );
         } else {
            message.error(
               "Gửi Email thanh toán thất bại. Vui lòng thử lại sau!"
            );
            return;
         }

         const response = await paidBooking(baseId);
         console.log("response ", response);

         if (response.status === 200) {
            message.success("Thanh toán đơn đặt thành công");
         } else {
            message.error("Thanh toán đơn đặt thất bại");
            return;
         }
         // Cập nhật lại danh sách đơn đặt sau khi xác nhận thành công
         fetchBookings();
   ...
   return (
      <>
         ...
         {/* Giao diện thanh toán đơn đặt */}
         <Modal
            title={
               <div className="flex items-center gap-1">
                  <h3>Thanh toán đơn đặt của</h3>
                  <h3
            ...
            onCancel={handleClosePaidModal}
            footer={
               <div className="flex items-center justify-end gap-2">
                  <Button ghost danger size="large" onClick={handlePaidBooking}>
                     Hủy
                  </Button>
                  <Button
            ...
            open={isShowPaidModal}
         >
            <div>
               <div className="flex items-center gap-1">
                  <p>Bạn có chắc chắn thanh toán chuyến du lịch ở</p>
                  <p
                     title={currentBooking?.dayDetail?.tour?.tourName}
               ...
               <div className="flex items-center gap-1 mt-3">
                  <p>Tổng giá tiền: </p>
                  <p
                     title={formatMoney(padiTotalPrice)}
                     className="format font-semibold text-amber-500"
                  >
                     {formatMoney(padiTotalPrice)}
                  </p>
               </div>
            </div>
         </Modal>
         ...
         {/* Giao diện header của hóa đơn */}
         <div className="mb-4">
            <h3 className="text-[24px] font-semibold">Hóa đơn</h3>
         </div>
